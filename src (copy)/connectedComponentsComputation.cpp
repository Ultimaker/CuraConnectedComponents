...
//             DEBUG_PRINTLN(f.p0 <<", "<<f.p1<<", "<<f.p2);
        ...
        for (int v = 0; v< cr.verts.length; v++)
        {
            const float* vs = reinterpret_cast<const float*>(cr.verts.data) + v*3 ;
            DEBUG_PRINTLN(vs[0] << ", " << vs[1] << ", " << vs[2]);
        }
        DEBUG_PRINTLN("faces RAW:");
        for (int v = 0; v< cr.faces.length; v++)
        {
            const int* vs = reinterpret_cast<const int*>(cr.faces.data) + v*3 ;
            DEBUG_PRINTLN(vs[0] << ", " << vs[1] << ", " << vs[2]);
        }
    }
